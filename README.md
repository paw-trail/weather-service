...
웹 화면의 날씨 띠가 사용자가 고른 지역으로 이 서비스를 부릅니다. DB 는 없고, 받은 예보를 Redis 에 잠시 담아 둡니다.
...
웹 화면에서 이 서비스를 부르는 곳은 머리띠 아래의 **날씨 띠** 하나입니다.

| 화면 | 넘기는 값 | 쓰는 방식 |
|---|---|---|
| 날씨 띠 — 메인 · 검색 · 장소 상세 · 일정 · 최근 본 장소 | 사용자가 고른 지역(`sidoCode` · `sigunguName`) — 시군구를 고르지 않았으면 `sidoCode` 만 | 고른 지역의 지금 날씨 · 오늘 비 · 눈 예보가 있으면 경고 ([4-3](#4-3-비-예보-경고)) |

---

**웹 화면은 내 위치(좌표)를 보내지 않습니다.** 지역은 사용자가 고르고, 처음에는 정해 둔 고정 지역에서 시작합니다.
좌표로 부르는 입구(`lat` · `lon`)는 장소 좌표를 기준으로 날씨를 볼 때 쓰도록 남겨 두었습니다. 까닭은 [11장](#11-왜-이렇게-만들었나) 에, 두 방식의 파라미터는 [7장](#7-api) 에 있습니다.
...
> **이 두 줄은 infra 레포의 `.env` 를 그대로 씁니다.** weather-service 는 `app` 프로파일에 있지만, 명령에 서비스 이름을 적으면
> 그 프로파일이 저절로 켜져서 `.env` 의 `COMPOSE_PROFILES` 에 `app` 이 없어도 됩니다. Windows · macOS 모두 같습니다.
>
> **`--profile app` 을 붙이면 안 됩니다.** 명령의 `--profile` 은 `.env` 값에 더해지는 것이 아니라 통째로 대체해서, 켜진 프로파일이 `app` 하나가 됩니다.
> 그러면 다른 서비스의 `depends_on` 이 가리키는 컨테이너가 프로젝트에 없다며 멈춥니다(앞의 서비스 이름은 그때그때 다릅니다).
>
> ```
> service "place-service" depends on undefined service "postgres": invalid compose project
> ```
>
> 프로파일을 한 번만 바꿔 부르려면 [1-2](#1-2-인프라-컨테이너) 처럼 필요한 프로파일을 모두 적습니다.
> 규칙은 [infra README 3장](https://github.com/paw-trail/infra#3-프로파일--무엇을-띄울지-고르기) 에 있습니다.
...
| 내일로 넘어간 예보는 세지 않음 | 날씨 띠가 알리려는 것은 오늘 나갈 때의 비입니다 |

### 4-3. 비 예보 경고

경고는 화면의 날씨 띠가 띄웁니다. 이 서비스는 `rainToday` 를 돌려줄 뿐이고, 판정(verdict)을 비롯한 다른 서비스를 부르지 않습니다.

| 고른 지역의 응답 | 날씨 띠 |
|---|---|
| `rainToday` 가 있음 | 경고 색으로 바뀌어 "종로구 오늘 오후 6시부터 비 예보 — 실내 동반 가능한 곳 미리 확인하세요!" 처럼 알림 · 닫으면 그날은 다시 띄우지 않음 |
| `rainToday` 가 `null` | 지금 기온 · 하늘 · 강수 확률과 기준 시각을 적음 |

장소 상세 화면도 같은 날씨 띠를 씁니다. 장소 좌표로 따로 부르지 않습니다.
...
| 방식 | 파라미터 | 부르는 곳 |
|---|---|---|
| 좌표 | `lat` · `lon` (둘 다) | 웹 화면은 부르지 않음 — 장소 좌표를 기준으로 날씨를 볼 때 쓰는 입구 ([11장](#11-왜-이렇게-만들었나)) |
| 지역 | `sidoCode` (+ `sigunguName`) | 날씨 띠 — `sigunguName` 을 빼면 시도 대표 |
...
| 웹 화면은 고른 지역으로만 부름 | 내 위치를 서버로 보내지 않기 위해서입니다 — 표 아래에 따로 적습니다 |
...
| 한 경로 두 방식 | 좌표로 부르든 지역으로 부르든 같은 모양의 응답을 받습니다. 함께 주면 400 인 것은 place 의 색인 조회와 같은 규칙입니다 |
...
**웹 화면은 내 위치를 서버로 보내지 않습니다.** 공모전 안내(FAQ)에 따르면 개인의 위치 정보를 서비스의 서버로 보내면, 서버가 저장하지 않더라도 위치기반서비스사업자 신고 대상이 됩니다.
그래서 날씨 띠는 브라우저의 위치가 아니라 사용자가 고른 지역으로 날씨를 묻습니다.

| 방식 | 서버로 가는 것 | 웹 화면 |
|---|---|---|
| 지역 | `sidoCode` · `sigunguName` | 날씨 띠가 이것만 씁니다 |
| 좌표 | `lat` · `lon` | 쓰지 않습니다 |

**좌표 입구는 남겨 둡니다.** 사람의 위치가 아니라 장소의 좌표를 넘겨 그 장소의 날씨를 볼 때 쓰는 입구입니다.
검색 서비스(search-service)의 위치 파라미터도 같은 방침으로 남겨 두었습니다.
...
| IP 로 지역 추정 | 서버는 사용자의 위치를 다루지 않습니다([11장](#11-왜-이렇게-만들었나)). 지역은 사용자가 고르고, 추정은 부정확합니다 |
...
| 용어 | 뜻 | 자세히 |
|---|---|---|
| 날씨 띠 | 웹 화면 머리띠 아래의 날씨 한 줄 — 이 서비스를 부르는 유일한 화면 · 고른 지역으로 부름 | [0-2](#0-2-부르는-화면) |
